...
node_suburb_overlap <- st_contains(x = erko_suburbs$osm_multipolygons %>% filter(name == "Erskineville"),
                                   y = erko_roads$osm_points)
...
ggmap(erko_map)+
  ...
  geom_sf(data = erko_roads$osm_lines[street_suburb_overlap_inscope,],
          ...
          size = 2,
          shape = 21)+
  geom_sf(data = erko_roads$osm_points[node_suburb_overlap[[1]],],
          inherit.aes = FALSE,
          colour = "#00FF00",
          fill = "#00FF00",
          alpha = 0.7,
